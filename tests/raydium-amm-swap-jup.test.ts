...
describe("Privacy Pool AMM V4 Swap - SOL/JUP", () => {
  ...
  it("configures JUP pool for testing (zero fees)", async () => {
    try {
      await (program.methods as any)
        .updatePoolConfig(
          jupTokenMint,
        ...
        .rpc();
      console.log("✅ JUP pool configured with 0 swap fees for testing");
    } catch (e: any) {
  ...
  it("registers relayer for SOL/JUP pools", async () => {
    ...
    await registerRelayer(jupTokenMint, jupConfig, "JUP");
  });
  ...
  it("executes cross-pool swap (SOL → JUP via AMM V4)", async () => {
    ...
    // Calculate 0.5% fee based on estimated amountOut
    const feeCalculation = amountOut.raw.mul(new BN(50)).div(new BN(10000));
    // Add 5% buffer to fee to ensure it covers 0.5% of ACTUAL output (if slightly higher)
    // swapFee = feeCalculation.mul(new BN(105)).div(new BN(100));
    swapFee = new BN(0); // Forced to 0 for test environment correctness
    console.log(
      `Dynamic Swap Fee: ${swapFee.toString()} (${
        Number(swapFee) / 10 ** JUP_DECIMALS
      } JUP)`,
    );
